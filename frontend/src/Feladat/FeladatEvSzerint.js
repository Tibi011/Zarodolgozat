...
const FeladatEvSzerint = ({ kivalasztott }) => {
  ...
  const szazalek = Math.round((db / adatok.length) * 100);

  MySwal.fire({
  title: "Értékelés",
  html: `
    <p><strong>Összes feladat:</strong> ${adatok.length}</p>
    <p><strong>Helyes válaszok:</strong> ${db}</p>
    ...
    const leToltes = async () => {
      try {
        ...
        if (response.ok) {
          ...
          // biztosítjuk, hogy minden elemhez legyen gombok tömb (6 hosszú)
