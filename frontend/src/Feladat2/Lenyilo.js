
import { useState,useEffect } from "react"
import Cim from "../Cim"

const Lenyilo=({kivalasztott,selectedYear})=>{
    const [adatok,setAdatok]=useState([])
    const [tolt,setTolt]=useState(true)
    const [hiba,setHiba]=useState(false)

    

    const darabol =(szoveg)=>{
        let kecske=szoveg.split(';')
        kivalasztott(kecske[0])
        selectedYear(kecske[1])
    }

    useEffect(()=>{

        const leToltes=async ()=>{
        try{
            const response=await fetch(Cim.Cim+"/tema")
            const data=await response.json()
            //alert(JSON.stringify(data))
            //console.log(data)
            if (response.ok)
                {
                    setAdatok(data)
                    setTolt(false)
                    kivalasztott(data[0].tema_id)
                    selectedYear(data[0].tema)
                }
            else 
                {
                    setHiba(true)
                    setTolt(false)
                }
            }
        catch (error){
            console.log(error)
            setHiba(true)
        }
        
    }



        leToltes()
    },[kivalasztott,selectedYear])

    if (tolt)
        return (
            <div style={{textAlign:"center"}}>Adatok betöltése folyamatban...</div>
                )
    else if (hiba)
        return (
            <div>Hiba</div>
                )       
    
    else return (
        <div>
            <select onChange={(e)=>  darabol(e.target.value)      }>
                {adatok.map((elem,index)=>(
                    <option key={index} value={elem.tema_id + ";" + elem.tema}> {elem.tema} </option>
                ))}
            </select>
        </div>
    )
}
export default Lenyilo
